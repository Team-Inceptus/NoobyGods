...
module.exports = {
	...
	.setName('getrank')
	.setDescription('Get a player\'s Guild Score.')
	...
	.addStringOption(option => 
		...
		.setDescription('The player\'s skyblock cute name.')
		.setRequired(false)),
	async run(client, interaction) {
		...
		function removeChatColor(str) {
			...
			let sbready = true;
			// Calculating Rank
			await hypixel.getPlayer(interaction.options.getString('username'))
			.then(async player => {
			...
			const profM = interaction.options.getString('profile') ? await hypixel.getSkyblockMember(interaction.options.getString('username')) : null;

			if (profM !== null && interaction.options.getString('profile')) {
				sbready = false;
				sbused = true;
				let profs = [];
				const profiles = await hypixel.getSkyblockProfiles(interaction.options.getString('username'));
				let i = 0;
				while (i < profiles.length) {
					profs.push(profiles[i].profileName);
					i++;
				}
				let argsOneCapitalized = interaction.options.getString('profile').toLowerCase().charAt(0).toUpperCase() + interaction.options.getString('profile').slice(1);
	...
	.catch(async err => {
			console.error(err);
			interaction.channel.send('This player does not exist, or does not have their API on. Please click here if you do not know how to turn your API on.\n<https://sky.lea.moe/resources/video/enable_api.webm>');
		})
	}
}